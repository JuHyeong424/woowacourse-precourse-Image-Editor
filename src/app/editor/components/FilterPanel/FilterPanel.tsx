import React from "react";
import ButtonFilters from "@/app/editor/components/FilterPanel/button/ButtonFilters";
import SliderFilters from "@/app/editor/components/FilterPanel/slider/SliderFilters";
import {FilterProps} from "@/app/types/filterStateTypes";
import AIFilter from "@/app/editor/components/FilterPanel/AIFilter";

interface ExtendedFilterProps extends FilterProps {
  aiLoading: boolean;
  aiError: string | null;
  runAutoEnhance: () => void;
}

export default function FilterPanel({ filters, setFilter, disabled, aiLoading, aiError, runAutoEnhance }: ExtendedFilterProps) {
  return (
    <aside className="flex flex-col p-4 border-2 min-w-[30%] small:h-[30%] medium:h-[30%] tablet:h-full laptop:h-full rounded-xl overflow-y-auto overlay-scroll">
      <h2 className="small:text-xl medium:text-xl tablet:text-2xl laptop:text-2xl text-center font-bold small:m-3 medium:m-3 tablet:m-4 laptop:m-4">편집 도구</h2>
      <AIFilter
        disabled={disabled}
        aiLoading={aiLoading}
        aiError={aiError}
        runAutoEnhance={runAutoEnhance}
      />
      <ButtonFilters
        filters={filters}
        setFilter={setFilter}
        disabled={disabled}
      />
      <SliderFilters
        filters={filters}
        setFilter={setFilter}
        disabled={disabled}
      />
    </aside>
  )
}
